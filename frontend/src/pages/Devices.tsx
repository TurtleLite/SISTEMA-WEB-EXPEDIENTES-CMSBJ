    ...
    <div className="h-full flex flex-col gap-2.5 min-h-0">
      {!embedded && (
        <div className="flex items-center justify-between shrink-0">
          <h1 className="font-serif text-xl font-bold text-[#3F4650]">Equipos</h1>
          <button
            onClick={load}
            className="px-2.5 py-1.5 text-xs text-[#5F6B80] bg-slate-50 border border-[#E3E6EB] rounded-lg hover:bg-slate-100 transition-all duration-200 flex items-center gap-1.5"
          >
            <RefreshCw size={13} />
            Actualizar
          </button>
      ...
      <div className="shrink-0 flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1 bg-slate-100 p-0.5 rounded-lg">
          {FILTER_OPTIONS.map((o) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all duration-200 ${filter === o.key ? 'bg-white text-[#3F4650] shadow-sm' : 'text-slate-500 hover:text-[#3F4650]'}`}
            >
        ...
        <div className="relative flex-1 min-w-40">
          ...
          <input
            ...
            className="w-full pl-8 pr-2.5 py-1.5 border border-[#E3E6EB] rounded-lg text-xs bg-white focus:ring-2 focus:ring-slate-300/30 focus:border-slate-400 transition-all duration-200"
          />
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#E3E6EB] flex flex-col min-h-0 flex-1">
          ...
          <table className="w-full table-fixed">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-100 border-b border-[#E3E6EB]">
                <th className="w-[21%] text-left px-3 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Equipo</th>
                <th className="w-[12%] text-left px-3 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Estado</th>
                <th className="w-[14%] text-left px-3 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Usuarios</th>
                <th className="w-[11%] text-left px-3 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Actividad</th>
                <th className="w-[7%] text-left px-3 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Eventos</th>
                <th className="w-[12%] text-left px-3 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Nota</th>
                <th className="w-[23%] text-right px-3 py-2.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Acción</th>
              </tr>
            </thead>
            <tbody>
              {visible.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-sm text-slate-400">
                    <Monitor size={26} className="mx-auto mb-2 text-slate-200" />
                  ...
                  <tr key={d.id} className={`border-b border-l-4 border-slate-100 ${meta.border} transition-all duration-150 hover:bg-slate-100/50`}>
                    <td className="px-3 py-2.5 min-w-0">
                      <div className="flex items-center gap-1.5 min-w-0">
                        ...
                        <span className="text-xs font-mono text-slate-800 truncate" title={`Primer uso: ${fmt(d.first_seen_at)}`}>{d.device_id}</span>
                        {d.shared && (
                          <span
                      ...
                      {d.note && <p className="text-[10px] text-slate-400 mt-0.5 truncate" title={d.note}>{d.note}</p>}
                    </td>
                    ...
                    <td className="px-3 py-2.5 min-w-0">
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        {d.users.length === 0 ? (
                          ...
                          shownUsers.map((u) => (
                            <span key={u} className="px-1 py-0.5 rounded bg-slate-100 text-[10px] text-slate-700 truncate max-w-full" title={`Primer uso: ${fmt(d.first_seen_at)}`}>{u}</span>
                          ))
                        )}
                        {extraUsers > 0 && <span className="text-[10px] text-slate-400" title={`${d.users.join(', ')}`}>+{extraUsers}</span>}
                      </div>
                    </td>
                    <td className="px-3 py-2.5 text-xs text-slate-600 min-w-0">
                      <p className="truncate" title={`Última actividad: ${fmt(d.last_event_at || d.last_seen_at)}`}>{timeAgo(d.last_event_at || d.last_seen_at)}</p>
                    </td>
                    <td className="px-3 py-2.5 text-xs text-slate-600">{d.events}</td>
                    <td className="px-3 py-2.5 min-w-0">
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          className="w-full min-w-0 px-1.5 py-1 border border-[#E3E6EB] rounded text-[10px] focus:ring-2 focus:ring-slate-300/30 focus:border-slate-400"
                        />
                        <button
                          onClick={() => saveNote(d)}
                          disabled={noteSaving === d.device_id}
                          className="p-1 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded disabled:opacity-40 shrink-0"
                          title="Guardar nota"
                        >
                          <Save size={12} />
                        </button>
        ...
        <div className="shrink-0 border-t border-[#E3E6EB] px-3 py-1.5 flex items-center justify-between">
